...
var fallbackImageContent []byte
var fallbackImageHash string

func init() {
	fallbackImageContent, err := os.ReadFile(fallbackImage)
	if err != nil {
		panic(err)
	}
	fallbackImageHash = fmt.Sprintf("%x", sha256.Sum256(fallbackImageContent))
	fmt.Println("@@@ ", "fallbackImageHash", fallbackImageHash)
}
...
func getIconHandler(c echo.Context) error {
	...
	// }
	userID, err := repoGetUserIDByUserName(ctx, tx, username)
	if err != nil {
	...
	// user, err := repoGetUserByID(ctx, tx, userID)
	// if err != nil {
	// 	return echo.NewHTTPError(http.StatusInternalServerError, "failed to get user: "+err.Error())
	// }
	imageHash, err := repoGetImageHashByUserID(ctx, tx, userID)
	if err != nil {
	...
	image, err := repoLoadImage(imageHash)
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to load image: "+err.Error())
	}
...
func postIconHandler(c echo.Context) error {
	...
	imageHashHex := hex.EncodeToString(imageHash[:])

	rs, err := tx.ExecContext(ctx, "INSERT INTO icons (user_id, image) VALUES (?, ?)", userID, imageHashHex)
	if err != nil {
	...
	if err := repoDumpImage(imageHashHex, req.Image); err != nil {
	...
	userCache.IDToImageHash.Delete(userID)
...
func getMeHandler(c echo.Context) error {
	...
	userModel, err := repoGetUserByID(ctx, tx, userID)
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get user: "+err.Error())
	}
...
func loginHandler(c echo.Context) error {
	...
	userID, err := repoGetUserIDByUserName(ctx, tx, req.Username)
	if err != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, "failed to get user id: "+err.Error())
	}
	userModel, err := repoGetUserByID(ctx, tx, userID)
	if err != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, "failed to get user: "+err.Error())
	}
...
func fillUserResponse(ctx context.Context, tx *sqlx.Tx, userModel UserModel) (User, error) {
	themeModel, err := repoGetThemeByUserID(ctx, tx, userModel.ID)
	if err != nil {
		return User{}, err
	}

	iconHash, err := repoGetImageHashByUserID(ctx, tx, userModel.ID)
	if err != nil {
